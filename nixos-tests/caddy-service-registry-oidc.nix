  ...
  mkEvalCheck {
    ...
    assertions = [
      ...
      }
      {
        ...
        assertion =
          (builtins.head foundryRoutes).match
          == [
            {
              host = ["vtt.example.com"];
              path = ["/api" "/api/*"];
            }
          ];
      ...
      }
      {
        ...
        assertion = (lib.last foundryRoutes).match == [{host = ["vtt.example.com"];}];
        message = "expected the final route to remain the host fallback";
      }
      {
        name = "foundry-fallback-backend";
        assertion = (upstream (lib.last foundryRoutes)).upstreams == [{dial = "192.0.2.20:8030";}];
